...
export function Solutions() {
  return <section id="solutions" className="py-20 md:py-28">
    <Container>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{homeServices.map(({ icon: Icon, title, description, href, label }, index) => <Link href={href} key={href} className="group flex min-h-[300px] flex-col justify-between rounded-lg border border-edge bg-panel p-7 transition-transform hover:-translate-y-1 hover:border-accent hover:shadow-[0_20px_45px_#40005b12]">
          <div className="flex items-start justify-between">
            <span className="grid size-12 place-items-center rounded-lg bg-selected text-link">
          ...
          <span className="inline-flex items-center gap-2 text-sm font-bold text-highlight">
            {label}
