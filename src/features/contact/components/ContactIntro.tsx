export function ContactIntro({ review = false }: { review?: boolean }) {
  ...
  return <div>
    <p className="mb-5 text-xs font-extrabold tracking-widest text-accent">{review ? '15-MINUTE AUTOMATION REVIEW' : 'START A CONVERSATION'}</p>
    <h1 className="text-[clamp(2.8rem,4.5vw,4.6rem)] leading-[1.08] font-bold tracking-[-0.055em]">{review ? <>Bring one workflow. Leave with <span className="text-accent">a clearer plan.</span></> : <>Tell me what is still being done <span className="text-accent">manually.</span></>}</h1>
    ...
    <div className="mt-9 border-t border-[#e7dbe9]">{points.map((point, index) => <div key={point} className="border-b border-[#e7dbe9] py-4 text-sm font-bold text-brand">0{index + 1} / {point}</div>)}</div>
