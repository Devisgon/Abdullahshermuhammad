...
export function OfferVisual({ kind }: { kind: OfferPage['visual'] }) {
  ...
  return <div className="overflow-hidden rounded-lg border border-edge-strong bg-panel shadow-[0_24px_60px_#40005b17]" aria-label={`Illustrative ${kind} workflow`}>
    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-edge bg-subtle p-5 text-[10px] font-bold tracking-wider text-link">
    ...
    <div className="p-6 md:p-8">
      ...
      {visual.items.map((item, index) => <div key={item} className="flex items-center gap-4 border-b border-edge py-4">
        <span className="text-xs font-bold text-highlight">0{index + 1}</span>
        <span className="flex-1 text-sm font-medium">{item}</span>
        <Check size={16} className="text-highlight" />
      </div>)}
    </div>
    <div className="flex items-center justify-between bg-subtle px-6 py-4 text-xs font-semibold text-link">Designed around your actual process. <ArrowRight size={16} /></div>
  </div>;
}
