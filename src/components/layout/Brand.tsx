...
export function Brand({ footer = false }: { footer?: boolean }) {
  return <Link href="/" aria-label="Abdullah Sher Muhammad home" className="inline-flex shrink-0 items-center gap-3 whitespace-nowrap">
    ...
    <span className={`${footer ? 'flex' : 'hidden min-[520px]:flex'} flex-col gap-1 leading-none`}>
      <strong className={`text-[11px] font-extrabold tracking-[0.06em] sm:text-sm ${footer ? 'text-white' : 'text-ink'}`}>ABDULLAH SHER</strong>
      <small className={`text-[7px] font-extrabold tracking-[0.12em] sm:text-[9px] ${footer ? 'text-[#e4acd5]' : 'text-highlight'}`}>AI SYSTEMS · DEVISGON</small>
    </span>
  </Link>;
}
